...
export interface AppState {
  ...
  setGenerateResult: (resultId: string | null, originalUrl: string | null, generatedUrl: string | null) => void;
  ...
  persist(
    (set) => ({
      ...
      setGenerateResult: (resultId, originalUrl, generatedUrl) =>
        set({ generateResultId: resultId, originalImageUrl: originalUrl, generatedImageUrl: generatedUrl }),
      setAiComment: (comment) => set({ aiComment: comment }),
      setPersonality: (p) => set({ personality: p }),
      decrementFreeCount: () => set((state) => ({ dailyFreeCount: Math.max(0, state.dailyFreeCount - 1) })),
      resetFlow: () =>
        set({
          uploadedImage: null,
          croppedImage: null,
